...
- Testimonial: she is confident, glowing, radiating the result. She looks like the product WORKS — adapt result indicators to the niche: SALUD = relaxed posture, free movement, picks something up off the floor without wincing | BELLEZA = glowing skin, no makeup needed, light caught on cheekbone | TECNOLOGÍA = phone at 80% late afternoon, no charging anxiety, scrolls without battery drop | HOGAR = clean surface visible behind her, satisfied after-shot of the cleaned area | WELLNESS = energized body language at 3pm, no afternoon slump, walks with intent. Dialogue includes a time anchor ("llevo 2 semanas usándolo") and before/after verbal contrast — niche-adapted: SALUD "antes no podía ni agacharme, ahora miren" | BELLEZA "antes mi piel se veía apagada en cualquier foto, ahora no necesito filtros" | TECNOLOGÍA "antes el cel no me llegaba a la tarde, ahora me sobra batería" | HOGAR "antes tenía que tallar 20 minutos, ahora paso un trapo y listo" | WELLNESS "antes a las 3pm me derrumbaba, ahora rindo hasta la noche". Product appears early. Hook = transformation result from someone you want to look like.
...
- Unboxing COD: she is excited and relatable but still attractive — styled for camera, good lighting on her face. Build anticipation with beats by niche: SALUD = reads dosage/expiry on label, smells the formula, checks the leaflet | BELLEZA = checks ingredient list, holds bottle to light to see texture, swatches a tiny dab on back of hand | TECNOLOGÍA = feels weight, peels protective film, checks model number on box | HOGAR = reads usage instructions, smells the formula, checks fill volume on the back | WELLNESS = examines packaging quality, reads serving size, checks third-party seal. The reveal IS the hero visual. Her reaction must be genuine and specific to the niche: SALUD = "huele a mentol fresco, no a medicamento" | BELLEZA = "qué textura más sedosa, parece de marca importada" | TECNOLOGÍA = "uy, pesa más de lo que esperaba — se siente premium" | HOGAR = "el olor es suave, no como esos químicos fuertes" | WELLNESS = "mirá el sello de calidad, esto sí es serio" — combine with eyebrows up, mouth drops, leans forward, covers mouth. Product = "pictured in /image1" at reveal. Hook = package in hands or doorbell.
...
- CONTENT SAFETY (Nano Banana Pro / Seedream 4 image-prompt downstream): when the prompt describes any visible torso, lower back, midriff, shoulder, or limb skin on a person, ALWAYS specify upper-body clothing in the same descriptor. Never describe "bare lower back + denim jeans" or "bare arm + dark pants" without also stating the top. Patterns that work: "wearing a fitted gray cotton t-shirt and dark jeans, the right arm visible from short sleeve" / "wearing a white tank top, the right shoulder and upper back visible above the strap" / "wearing a hospital gown with the lower back zone exposed for examination" / "wearing a sports bra and athletic shorts, the abdominal zone visible between". Patterns that get rejected: "patient with bare arm wearing dark blue jeans" (no upper-body coverage) / "bare upper back visible, the patient seen from the side wearing dark pants" (mismatched coverage). Whenever a SALUD scene shows the user touching a pain zone (lower back, abdomen, shoulder, knee), specify the matching clothing that naturally exposes that zone — never let upper-body coverage be implicit.`;
